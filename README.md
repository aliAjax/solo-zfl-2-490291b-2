...
- **离线 + 双页合并**：在线时跨标签经 `localStorage` 的 `storage` 事件实时合并；离线改动进本页队列，恢复在线或手动“刷新”时裁决。
  ...
  - **盲测投票按测试员并集**：两个离线页不同测试员的合法票合并后双方都保留；同一测试员分叉的两张不同票不静默覆盖——先投保留、后投拦截并在裁决报告列出。评审团/样本按成员并集。
...
`e2e/spec.mjs`（真实 Chromium，Playwright，9 个场景）：建批与缺配方拦截、资源冲突、盲测越权、
异常隔离、失败整单回滚、双页字段合并（不同测试员离线投票并存 / 同人重复投票拦截 / 重复测量只吸收一次）、
导入导出与恶意导入拦截、预约与测量校验反例（坏日期、不存在引用、排队/无有效预约、撤销重做），
以及**真实公历日期 / 批次-样机装配关系 / 自装预约与自装测量（含异常路径不隔离）**。
