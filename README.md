...
- **预约完整性**：开始/结束时间必须可解析且结束晚于开始；批次、样机、机位、测试员（须为测试员角色）任一不存在，或耗材名称/数量非法，**整单拒绝、零写入**（不产生预约也不出库耗材）。
...
`e2e/spec.mjs`（真实 Chromium，Playwright，8 个场景）：建批与缺配方拦截、资源冲突、盲测越权、
异常隔离、失败整单回滚、双页字段合并（不同测试员离线投票并存 / 同人重复投票拦截 / 重复测量只吸收一次）、
导入导出与恶意导入拦截、以及**预约与测量的校验反例**（坏日期、不存在的批次/样机/机位/测试员、
排队或无有效预约时测量）和撤销重做。
